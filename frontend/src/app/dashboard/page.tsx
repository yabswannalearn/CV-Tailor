...
const labelClass = "block text-[10px] tracking-[0.25em] text-[#7a7570] uppercase mb-2";

type Education = { school_name: string; course: string; location: string };
type Experience = { name: string; company: string; location: string; description: string; date: string };
type Project = { name: string; description: string; date: string };

interface Profile {
  first_name: string; last_name: string; mobile_no: string; email: string;
  linkedin: string; github: string; portfolio: string;
  education: Education[]; experience: Experience[]; projects: Project[]; skills: string[];
}
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    fetch(`${API}/auth/me`, { credentials: "include" })
      ...
      .then(res => { if (!res || !res.ok) return null; return res.json(); })
      .then(data => {
        if (!data) return;
        setProfile({
          ...
          education: (data.education || []).map((e: any) => ({ school_name: e.school_name, course: e.course, location: e.location })),
          experience: (data.experience || []).map((e: any) => ({ name: e.job_title, company: e.company, location: e.location, description: e.description, date: e.date_range })),
          projects: (data.projects || []).map((p: any) => ({ name: p.name, description: p.description, date: p.date_range })),
  ...
  const updateField = (field: keyof Profile, value: any) => setProfile(p => ({ ...p, [field]: value }));
  const updateListItem = <T,>(field: keyof Profile, index: number, updated: T) =>
    setProfile(p => ({ ...p, [field]: (p[field] as T[]).map((item, i) => i === index ? updated : item) }));
  ...
  const removeListItem = (field: keyof Profile, index: number) =>
    setProfile(p => ({ ...p, [field]: (p[field] as any[]).filter((_, i) => i !== index) }));

  const tabs = ["personal", "education", "experience", "projects", "skills"] as const;
          ...
          {tabs.map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-4 py-2 text-[10px] tracking-[0.2em] uppercase transition-colors duration-200 border-b-2 -mb-px ${
                activeTab === tab ? "border-[#5a8a00] text-[#5a8a00]" : "border-transparent text-[#b0aba4] hover:text-[#7a7570]"
              }`}>
              {tab}
            </button>
          ))}
          ...
          <div className="mb-10">
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div><label className={labelClass}>First Name</label>
                <input className={inputClass} value={profile.first_name} onChange={e => updateField("first_name", e.target.value)} placeholder="John" /></div>
              <div><label className={labelClass}>Last Name</label>
                <input className={inputClass} value={profile.last_name} onChange={e => updateField("last_name", e.target.value)} placeholder="Doe" /></div>
            </div>
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div><label className={labelClass}>Mobile No</label>
                <input className={inputClass} value={profile.mobile_no} onChange={e => updateField("mobile_no", e.target.value)} placeholder="+63 912 345 6789" /></div>
              <div><label className={labelClass}>Email</label>
                <input className={inputClass} value={profile.email} onChange={e => updateField("email", e.target.value)} placeholder="you@email.com" /></div>
            </div>
            <div className="mb-4"><label className={labelClass}>LinkedIn</label>
              <input className={inputClass} value={profile.linkedin} onChange={e => updateField("linkedin", e.target.value)} placeholder="https://linkedin.com/in/..." /></div>
            <div className="mb-4"><label className={labelClass}>GitHub</label>
              <input className={inputClass} value={profile.github} onChange={e => updateField("github", e.target.value)} placeholder="https://github.com/..." /></div>
            <div><label className={labelClass}>Portfolio</label>
              <input className={inputClass} value={profile.portfolio} onChange={e => updateField("portfolio", e.target.value)} placeholder="https://yoursite.dev" /></div>
          </div>
        )}
          ...
          <div className="mb-10">
            {profile.education.map((edu, i) => (
              <div key={i} className="mb-6 p-4 bg-[#eeebe5] border border-[#d4cfc7] rounded-sm relative">
                ...
                <div className="mb-3"><label className={labelClass}>School Name</label>
                  <input className={inputClass} value={edu.school_name}
                    onChange={e => updateListItem("education", i, { ...edu, school_name: e.target.value })} placeholder="University Name" /></div>
                <div className="grid grid-cols-2 gap-4">
                  <div><label className={labelClass}>Course / Degree</label>
                    <input className={inputClass} value={edu.course}
                      onChange={e => updateListItem("education", i, { ...edu, course: e.target.value })} placeholder="BS Computer Engineering" /></div>
                  <div><label className={labelClass}>Location</label>
                    <input className={inputClass} value={edu.location}
                      onChange={e => updateListItem("education", i, { ...edu, location: e.target.value })} placeholder="City, Country" /></div>
                </div>
              </div>
            ))}
            <button onClick={() => addListItem("education", { school_name: "", course: "", location: "" })}
                ...
                <div className="grid grid-cols-2 gap-4 mb-3">
                  <div><label className={labelClass}>Job Title</label>
                    <input className={inputClass} value={exp.name}
                      onChange={e => updateListItem("experience", i, { ...exp, name: e.target.value })} placeholder="Software Engineer" /></div>
                  <div><label className={labelClass}>Company</label>
                    <input className={inputClass} value={exp.company}
                      onChange={e => updateListItem("experience", i, { ...exp, company: e.target.value })} placeholder="Company Name" /></div>
                </div>
                <div className="grid grid-cols-2 gap-4 mb-3">
                  <div><label className={labelClass}>Location</label>
                    <input className={inputClass} value={exp.location}
                      onChange={e => updateListItem("experience", i, { ...exp, location: e.target.value })} placeholder="Remote / City" /></div>
                  <div><label className={labelClass}>Date</label>
                    <input className={inputClass} value={exp.date}
                      onChange={e => updateListItem("experience", i, { ...exp, date: e.target.value })} placeholder="Jan 2024 – Present" /></div>
                </div>
                <div><label className={labelClass}>Description</label>
                  <textarea className={`${inputClass} resize-none h-24`} value={exp.description}
                    onChange={e => updateListItem("experience", i, { ...exp, description: e.target.value })} placeholder="What you did and its impact..." /></div>
              </div>
            ))}
            ...
            {profile.projects.map((proj, i) => (
              <div key={i} className="mb-6 p-4 bg-[#eeebe5] border border-[#d4cfc7] rounded-sm relative">
                <button onClick={() => removeListItem("projects", i)}
                  className="absolute top-3 right-3 text-[#b0aba4] hover:text-[#cc3333] text-xs transition-colors">✕</button>
                <div className="grid grid-cols-2 gap-4 mb-3">
                  <div><label className={labelClass}>Project Name</label>
                    <input className={inputClass} value={proj.name}
                      onChange={e => updateListItem("projects", i, { ...proj, name: e.target.value })} placeholder="Project Name" /></div>
                  <div><label className={labelClass}>Date</label>
                    <input className={inputClass} value={proj.date}
                      onChange={e => updateListItem("projects", i, { ...proj, date: e.target.value })} placeholder="2024" /></div>
                </div>
                <div><label className={labelClass}>Description</label>
                  <textarea className={`${inputClass} resize-none h-24`} value={proj.description}
                    onChange={e => updateListItem("projects", i, { ...proj, description: e.target.value })} placeholder="What you built and its impact..." /></div>
              </div>
            ))}
